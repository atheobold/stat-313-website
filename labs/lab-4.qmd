...
Today we'll be working with data on lake ice duration for two lakes in Madison, Wisconsin. The dataset contains information on the number of days of ice on each lake for years between 1851 and 2019. These data are stored in the `ntl_icecover` dataset, which lives in the **lterdatsampler** package.
...
```{r scatterplot}
# Code to create a scatterplot goes here!
...
**Question 4** -- Add a linear regression line to the scatterplot you made above.
...
**Question 6** -- Fit a linear regression modeling the relationship between between `ice_duration` and `year`. Save your linear regression into an object (i.e., use the assignment arrow `<-`) so you can use it later.

```{r lm}
# Code to fit a linear regression between ice_duration and year
...
**Question 7** -- Use the `get_regression_table()` function to obtain the coefficient estimates for the regression you fit in Question 6.
...
**Question 9** -- Interpret the value of the slope coefficient. Namely, what do you expect to happen to the duration of ice if the number of years is increased by 100?
...
**Question 10** -- Using the code you wrote for Question 2 (and 4), add a `color` for the name of the lake (`lakeid`).

```{r adding-color}
# Copy and paste your code from Question 2 and add a color for lakeid
