...
**2. Create a scatterplot modeling the relationship between latitude (explanatory) and water temperature (response).**
...
**4. Use the `get_correlation()` function to calculate the correlation between the water temperature and latitude.**

```{r calculate-correlation}

```

**5. Fill in the code below to calculate the slope for the relationship between the water temperature (response) and latitude (explanatory).**
...
```{r boot-dist}
#| cache: true
# The cache: true option stores your distribution so you don't get a new one every time you run the code! 
...
**11. How do your confidence intervals compare? Why do you believe they are similar or different?**
