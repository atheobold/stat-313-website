...
subtitle: "Due May 28, 2023 by midnight"
format: 
  html:
...
1. fit a multiple linear regression with **both** variables included (e.g., `my_model <- lm(bill_length_mm ~ body_mass_g + flipper_length_mm, data = penguins)`)

2. run an ANOVA to test if each variable should be included (e.g., `anova(my_model)`)
:::
...
1. fit a different slopes multiple linear regression (e.g., `my_model <- lm(bill_length_mm ~ flipper_length_mm * species, data = penguins)`)

2. run an ANOVA to test for different slopes (e.g., `anova(my_model)`)
:::
:::
