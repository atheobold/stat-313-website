...
execute: 
  echo: false
resources: 
  - reading_guide/week7_reading_guide_sampling.docx
  - reading_guide/week7_reading_guide_sampling.html
---
...
Download the reading guide as a [Word Document here](reading_guide/week7_reading_guide_sampling.docx)

Download the reading guide as an [HTML file here](reading_guide/week7_reading_guide_sampling.html)
...
```{r distribution-of-student-proportions}
#| label: fig-student-proportions
...
Observe that `virtual_shovel` has `r nrow(virtual_shovel)` rows corresponding to our virtual sample of size `r nrow(virtual_shovel)`. The `ball_ID` variable identifies which of the `r nrow(bowl)` balls from `bowl` are included in our sample of `r nrow(virtual_shovel)` balls while `color` denotes its color. However, what does the `replicate` variable indicate? In `virtual_shovel`'s case, `replicate` is equal to 1 for all `r nrow(virtual_shovel)` rows. This is telling us that these `r nrow(virtual_shovel)` rows correspond to the first repeated/replicated use of the shovel, in our case our first sample. We'll see shortly that when we "virtually" take 33 samples, `replicate` will take values between 1 and 33.

Let's compute the proportion of balls in our virtual sample that are red using the **dplyr** data wrangling verbs you learned in Week 3. First, for each of our `r nrow(virtual_shovel)` sampled balls, let's identify if it is red or not using a test for equality with `==`. Let's create a new Boolean variable `is_red` using the `mutate()` function:
...
Second, let's compute the number of balls out of `r nrow(virtual_shovel)` that are red using the `summarize()` function. Recall from Week 3 that `summarize()` takes a data frame with many rows and returns a data frame with a **single** row containing summary statistics, like the `mean()` or `median()`. In this case, we use the `sum()` function:
...
```{r proportion-reds-object}
virtual_shovel_prop_red <- virtual_shovel %>%
  mutate(is_red = color == "red") %>%
  summarize(prop_red = sum(is_red) / 50) %>%
  pull(prop_red)

virtual_shovel_perc_red <- virtual_shovel_prop_red * 100
```

Great! `r virtual_shovel_perc_red`% of `virtual_shovel`'s `r nrow(virtual_shovel)` balls were red! So based on this particular sample of `r nrow(virtual_shovel)` balls, our guess at the proportion of the `bowl`'s balls that are red is `r virtual_shovel_perc_red`%. But remember from our earlier tactile sampling activity that if we repeat this sampling, we will not necessarily obtain the same value of `r virtual_shovel_perc_red`% again. There will likely be some variation. In fact, our 33 groups of friends computed 33 such proportions whose distribution we visualized in @fig-full-hist. We saw that these estimates *varied*. Let's now perform the virtual analog of having 33 groups of students use the sampling shovel!
...
Observe that now `virtual_samples` has 1000 $\times$ 50 = `r nrow(virtual_samples)` rows, instead of the 33 $\times$ 50 = 1650 rows from earlier. Using the same data wrangling code as earlier, let's take the data frame `virtual_samples` with 1000 $\times$ 50 = 50,000 rows and compute the resulting 1000 proportions of red balls.
...
So in our sampling activities, the **population** is the collection of $N$ = `r nrow(bowl)` identically sized red and white balls in the bowl shown in @fig-bowl. Recall that we also represented the bowl "virtually" in the data frame `bowl`:
...
In order to compute this population proportion $p$ exactly, we need to first conduct a **census** by going through all $N$ = `r nrow(bowl)` and counting the number that are red. We then divide this count by `r nrow(bowl)` to obtain the proportion red.
...
total_red <- summarize(bowl, 
          total_red = sum(color == "red")
          ) %>% 
  pull(total_red)

prop_red <- total_red / nrow(bowl)

```

Since `r total_red` of the `r nrow(bowl)` are red, the proportion is `r total_red`/`r nrow(bowl)` = `r prop_red` = `r prop_red * 100`%. So we know the value of the population parameter: in our case, the population proportion $p$ is equal to `r prop_red`.

At this point, you might be further asking yourself: "If we had a way of knowing that the proportion of the balls that are red is `r prop_red * 100`%, then why did we do any sampling?" Great question! Normally, you wouldn't do any sampling! However, the sampling activities we did this chapter are merely simulations of how sampling is done in real-life! We perform these simulations in order to study:
...
-   the sample proportion $\widehat{p}$ of the $n$ = 50 balls in the shovel that are red is a "good guess" of the population proportion $p$ of the bowl's $N$ = `r nrow(bowl)` balls that are red, thus
-   instead of conducting a *census* of the `r nrow(bowl)` balls in the bowl, we can **infer** about the bowl using the sample from the shovel.
...
```{r comparing-sampling-distributions-2}
...
Recall from earlier that the value of the population proportion $p$ of the $N$ = `r nrow(bowl)` balls in the bowl was `r total_red`/`r nrow(bowl)` = `r prop_red` = `prop_red * 100`%. We computed this value by performing a virtual census of `bowl`. Let's re-display our sampling distributions from @fig-comparing-sampling-distributions and @fig-comparing-sampling-distributions-2, but now with a vertical red line marking the **true** population proportion $p$ of balls that are red = `r prop_red * 100`% in @fig-comparing-sampling-distributions-3. We see that while there is a certain amount of error in the sample proportions $\widehat{p}$ for all three sampling distributions, on average the $\widehat{p}$ are centered at the true population proportion red $p$.

```{r comparing-sampling-distributions-3}
...
virtual_prop %>%
  ...
  facet_wrap(~n) +
  geom_vline(xintercept = p, col = "red", linewidth = 1.5)
```
...
all_points %>% 
  ...
  geom_line() +
  geom_text(data = for_labels, aes(label = Distribution),
                            nudge_x = c(-2, -1.5, 2)) +
  theme_light() +
...
ggplot(data = tibble(x = c(-4, 4)), 
  ...
  stat_function(fun = shade_1_sd, 
                geom = "area", 
                fill = "black", 
                alpha = 0.25, 
                n = 1000) +
  geom_vline(xintercept = c(-3, -1.96, -1, 0, 1, 1.96, 3), 
             linetype = "dashed", 
             alpha = 0.5) +
  # Axes
